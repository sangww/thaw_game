...
//--------------------------------------------------------------
void marioStage10::setup(){
    ofEnableAlphaBlending();
    
    thaw = thawAppManager::getInstance();
    
    //spritemanager
    stage.setup("settings.xml", "stage07.xml");
    stage.setEntry(true, true, true, true);
    
    bear.loadImage("bear_single.png");
    ...
    //mode of interaction
    marioMode = 0; // 0: here
    availableMode = 6; // 6: pinch mario
}

//--------------------------------------------------------------
void marioStage10::update(){
...
//--------------------------------------------------------------
void marioStage10::gotOSCMessage(ofxOscMessage m){
    ...
    else if(m.getAddress() == "/mario/client/pinch"){
        sizeMario = m.getArgAsFloat(0);
    }
}

//--------------------------------------------------------------
void marioStage10::draw(){
    ...
    if(ofGetSystemTime() - t_reset < 1000){
        ...
        ofSetColor(255);
        ofEnableSmoothing();
        if(marioMode>0 && sizeMario>3){
            bear.draw(thaw->phoneX + phoneMarioX/2 + 50 - 19*sizeMario, thaw->phoneY + phoneMarioY/2, bear.getWidth()*sizeMario, bear.getHeight()*sizeMario);
        }
        stage.draw(sizeMario,sizeMario);
    ...
    if(thaw->debug){
        ...
        ofDrawBitmapString("size : " + ofToString(sizeMario), 10, 240);
    }
}


//--------------------------------------------------------------
void marioStage10::keyPressed  (int key){
...
//--------------------------------------------------------------
void marioStage10::keyReleased  (int key){
    ...
    if(key == '['){
        sizeMario -= 0.25;
        if(sizeMario < 0.25) sizeMario = 0.25;
    }
}

//--------------------------------------------------------------
void marioStage10::mouseMoved(int x, int y ){
}

//--------------------------------------------------------------
void marioStage10::mouseDragged(int x, int y, int button){
}

//--------------------------------------------------------------
void marioStage10::mousePressed(int x, int y, int button){
}

//--------------------------------------------------------------
void marioStage10::mouseReleased(int x, int y, int button){
}

void marioStage10::reset(){
