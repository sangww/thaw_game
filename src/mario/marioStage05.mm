...
//--------------------------------------------------------------
void marioStage05::setup(){
    ofEnableAlphaBlending();
    
    thaw = thawAppManager::getInstance();
    
    //spritemanager
    stage.setup("settings.xml", "stage05.xml");
    ...
    //mode of interaction
    marioMode = 0; // 0: here
    availableMode = 4; // 1: inside, 2: water, 3: pipe, 4: force
...
//--------------------------------------------------------------
void marioStage05::update(){
    
	stage.update(thaw->phoneX, thaw->phoneY, thaw->phoneW, thaw->phoneH, thaw->angle);
    
    if(thaw->screenTouch && stage.get1X() > thaw->phoneX && stage.get1X() < thaw->phoneX + thaw->phoneW){
        stage.set1VY(stage.get1VY()-5);
    }
...
//--------------------------------------------------------------
void marioStage05::gotOSCMessage(ofxOscMessage m){
...
void marioStage05::draw(){
    ...
    if(ofGetSystemTime() - t_reset < 1000){
        ...
        ofSetColor(225);
        stage.assets->font.drawString("STAGE05", 500, 500);
    }
    else{
        //mario
        ofSetColor(255);
        ...
        if(thaw->onScreenMode && thaw->onTrackPattern && thaw->phoneY >600){
            ofSetColor(255);
            spike.draw(thaw->phoneX+0, thaw->phoneY-70, thaw->phoneW-50, 100);
        }
...
//--------------------------------------------------------------
void marioStage05::keyPressed  (int key){
...
//--------------------------------------------------------------
void marioStage05::keyReleased  (int key){
...
//--------------------------------------------------------------
void marioStage05::mouseMoved(int x, int y ){
}

//--------------------------------------------------------------
void marioStage05::mouseDragged(int x, int y, int button){
}

//--------------------------------------------------------------
void marioStage05::mousePressed(int x, int y, int button){
}

//--------------------------------------------------------------
void marioStage05::mouseReleased(int x, int y, int button){
}

void marioStage05::reset(){
